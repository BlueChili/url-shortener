...
db.run('CREATE TABLE IF NOT EXISTS urls ( id INTEGER PRIMARY KEY, url TEXT NOT NULL, code TEXT NOT NULL, hits INTEGER NOT NULL)');
const queryByUrl = db.query('SELECT * from urls WHERE url = ?');
const queryByCode = db.query('SELECT * from urls WHERE code = ?');
const insert = db.prepare('INSERT INTO urls (url, code, hits) VALUES ($url, $code, 0)');
...
app.post("/", async ({body, request, set}) => {
...
app.get('/:code', async ({ params }) => {
  ...
  return result !== null ? (
    <html lang="en">
      ...
      <body>
          <p>If you are not redirected, <a href={result.url}>click here</a>.</p>
      </body>
    </html>
  ) : '';
});
